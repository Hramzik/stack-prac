...
Stack_state  stack_damaged  (Stack* stack) {
    ...
    if (stack == nullptr) { stack_state |= NULLPTR_STACK; return stack_state; }


    if (stack->size > stack->capacity)                  { stack_state |= SIZE_GREATER_THAN_CAPACITY; }
    if (stack->elements == nullptr && stack->size != 0) { stack_state |= NULLPTR_ELEMENTS; }


    for (size_t i = 0; i < stack->capacity; i++) {

        if (i <  stack->size) if ( stack->elements [i].poisoned) { stack_state |= INCORRECT_POISON_DISTRIBUTION; break; }
        if (i >= stack->size) if (!stack->elements [i].poisoned) { stack_state |= INCORRECT_POISON_DISTRIBUTION; break; }
    }

    IF_CANARY_PROTECTED (

        if (stack-> FIRST_CANARY !=  FIRST_CANARY_VALUE) { stack_state |= STACK_CANARY_CORRUPTED;  }
        if (stack->SECOND_CANARY != SECOND_CANARY_VALUE) { stack_state |= STACK_CANARY_CORRUPTED;  }

        if ( *( (canary_t*) stack->elements - 1)                  !=  FIRST_CANARY_VALUE) { stack_state |= DATA_CANARY_CORRUPTED; }
        if ( *( (canary_t*) (stack->elements + stack->capacity) ) != SECOND_CANARY_VALUE) { stack_state |= DATA_CANARY_CORRUPTED; }
    );
